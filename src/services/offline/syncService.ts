...
class SyncService {
  ...
  private async syncPendingChanges(): Promise<void> {
    const pendingMutations = await db.pendingMutations
      .orderBy("createdAt")
      .toArray();
    ...
    for (const mutation of pendingMutations) {
      try {
        ...
        failureCount++;
        console.error(
          `Failed to sync ${mutation.operation} ${mutation.entityType}:`,
          error,
        );
        ...
        if (mutation.retryCount >= 5) {
          console.error(
            `Mutation ${mutation.id} has failed ${mutation.retryCount} times`,
          );
        }
      }
    }
  ...
  private async clearSyncedData(): Promise<void> {
    await db.transaction(
      "rw",
      [db.projects, db.activities, db.beneficiaries, db.subprojects],
      async () => {
