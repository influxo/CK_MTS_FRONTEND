...
} from "../activities/activityModels";
...
class OfflineDataService {
  ...
  async getProject(id: string): Promise<Project | null> {
    ...
    if (isOnline) {
      try {
        ...
        console.error(
          "Failed to fetch project from API, falling back to cache:",
          error,
        );
      }
    }
  ...
  async getActivity(id: string): Promise<Activity | null> {
    ...
    if (isOnline) {
      try {
        ...
        console.error(
          "Failed to fetch activity from API, falling back to cache:",
          error,
        );
      }
    }
  ...
  async createActivity(
    activityData: CreateSubprojectActivityRequest,
  ): Promise<Activity> {
    const isOnline = syncService.isOnline();
